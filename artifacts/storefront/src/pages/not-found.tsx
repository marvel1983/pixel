...
export default function NotFound() {
  ...
  useEffect(() => {
    fetch(`${API}/products?featured=1&limit=4&stock=1`)
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => { if (data?.items) setFeaturedProducts(data.items.map(toMockProduct)); })
      .catch(() => {});
  }, []);
    ...
    <div className="min-h-screen bg-background py-16 px-4">
      <div className="flex flex-col items-center justify-center text-center mb-16">
        ...
        <p className="text-muted-foreground max-w-md mb-8">
          {t("notFound.description", {
            defaultValue: "The page you're looking for doesn't exist or may have been moved. Let's get you back on track.",
          })}
        </p>
      ...
      {featuredProducts.length > 0 && (
        <div className="max-w-5xl mx-auto">
          <h3 className="text-xl font-bold text-center mb-8 text-foreground">Popular Products</h3>
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {featuredProducts.map((product) => {
              const variant = product.variants?.[0];
              if (!variant) return null;
              const price = parseFloat(variant.priceUsd);
              const comparePrice = variant.compareAtPriceUsd ? parseFloat(variant.compareAtPriceUsd) : null;
              return (
                  ...
                  <Card className="h-full hover:shadow-md transition-shadow cursor-pointer group">
                    <CardContent className="p-0 flex flex-col h-full">
                      ...
                      <div className="p-3 flex flex-col flex-1">
                        <p className="text-sm font-medium text-foreground line-clamp-2 mb-2 group-hover:text-primary transition-colors">{product.name}</p>
                        <div className="mt-auto flex items-baseline gap-1.5 flex-wrap">
                          ...
                          {comparePrice != null && (
                            <span className="text-xs text-muted-foreground line-through">{format(comparePrice)}</span>
                          )}
                        </div>
                      </div>
                    </CardContent>
