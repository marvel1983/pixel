import { Router, type IRouter } from "express";
import healthRouter from "./health";
import authRouter from "./auth";
import authProfileRouter from "./auth-profile";
import authGoogleRouter from "./auth-google";
import adminRouter from "./admin";
import adminDashboardRouter from "./admin-dashboard";
import adminAnalyticsRouter from "./admin-analytics";
import adminProductsRouter from "./admin-products";
import adminPlatformsRouter from "./admin-platforms";
import adminCategoriesRouter from "./admin-categories";
import adminOrdersRouter from "./admin-orders";
import adminOrdersActionsRouter from "./admin-orders-actions";
import adminOrdersDiagnoseRouter from "./admin-orders-diagnose";
import adminOrdersHoldsRouter from "./admin-orders-holds";
import adminOrdersJourneyRouter from "./admin-orders-journey";
import adminCustomerSessionsRouter from "./admin-customer-sessions";
import adminAnalyticsFunnelRouter from "./admin-analytics-funnel";
import meTrackingRouter from "./me-tracking";
import adminKeysRouter from "./admin-keys";
import adminClaimsRouter from "./admin-claims";
import adminDiscountsRouter from "./admin-discounts";
import adminCustomersRouter from "./admin-customers";
import adminReviewsRouter from "./admin-reviews";
import adminBannersRouter from "./admin-banners";
import adminPagesRouter from "./admin-pages";
import adminSettingsRouter from "./admin-settings";
import adminSettingsExtraRouter from "./admin-settings-extra";
import adminPaymentProvidersRouter from "./admin-payment-providers";
import adminWebhooksChatRouter from "./admin-webhooks-chat";
import adminAuditRouter from "./admin-audit";
import adminBalanceRouter from "./admin-balance";
import adminUpsellRouter from "./admin-upsell";
import adminHomepageRouter from "./admin-homepage";
import adminBrandsRouter from "./admin-brands";
import adminEmailTemplatesRouter from "./admin-email-templates";
import adminTaxRouter from "./admin-tax";
import adminUsersRouter from "./admin-users";
import adminRefundsRouter from "./admin-refunds";
import adminNotificationsRouter from "./admin-notifications";
import adminSeoTrackingRouter from "./admin-seo-tracking";
import adminGiftCardsRouter from "./admin-gift-cards";
import adminAffiliatesRouter from "./admin-affiliates";
import adminAbandonedCartsRouter from "./admin-abandoned-carts";
import adminAlertsRouter from "./admin-alerts";
import adminQaRouter from "./admin-qa";
import adminNewsletterRouter from "./admin-newsletter";
import adminFlashSalesRouter from "./admin-flash-sales";
import adminLoyaltyRouter from "./admin-loyalty";
import adminFeedsRouter from "./admin-feeds";
import adminBundlesRouter from "./admin-bundles";
import adminSupportRouter from "./admin-support";
import supportTicketsRouter from "./support-tickets";
import walletRouter from "./wallet";
import adminWalletRouter from "./admin-wallet";
import loyaltyRouter from "./loyalty";
import bundlesRouter from "./bundles";
import flashSalesRouter from "./flash-sales";
import alertsRouter from "./alerts";
import qaRouter from "./qa";
import newsletterRouter from "./newsletter";
import affiliatesRouter from "./affiliates";
import giftCardsRouter from "./gift-cards";
import abandonedCartsRouter from "./abandoned-carts";
import couponsRouter from "./coupons";
import ordersRouter from "./orders";
import orderInvoiceRouter from "./order-invoice";
import checkoutSessionRouter from "./checkout-session";
import checkoutOffersRouter from "./checkout-offers";
import orderLookupRouter from "./order-lookup";
import wishlistRouter from "./wishlist";
import currenciesRouter from "./currencies";
import webhooksRouter from "./webhooks";
import webhooksStripeRouter from "./webhooks-stripe";
import webhooksCheckoutRouter from "./webhooks-checkout";
import checkoutServicesRouter from "./checkout-services";
import adminCheckoutServicesRouter from "./admin-checkout-services";
import searchRouter from "./search";
import trustpilotRouter from "./trustpilot";
import socialProofRouter from "./social-proof";
import adminSocialProofRouter from "./admin-social-proof";
import i18nRouter from "./i18n";
import adminI18nRouter from "./admin-i18n";
import blogRouter from "./blog";
import adminBlogRouter from "./admin-blog";
import consentRouter from "./consent";
import quotesRouter from "./quotes";
import adminQuotesRouter from "./admin-quotes";
import surveysRouter from "./surveys";
import adminJobsRouter from "./admin-jobs";
import adminCircuitBreakerRouter from "./admin-circuit-breaker";
import productReviewsRouter from "./product-reviews";
import pricingRouter from "./pricing";
import adminPriceRulesRouter from "./admin-price-rules";
import adminAttributesRouter from "./admin-attributes";
import adminTagsRouter from "./admin-tags";
import adminUploadRouter from "./admin-upload";
import adminImportsRouter from "./admin-imports";
import adminMetenziCatalogRouter from "./admin-metenzi-catalog";
import adminMetenziMappingsRouter from "./admin-metenzi-mappings";
import adminMetenziSyncRouter from "./admin-metenzi-sync";
import adminMetenziWebhooksRouter from "./admin-metenzi-webhooks";
import adminMappingConflictsRouter from "./admin-mapping-conflicts";
import productsRouter from "./products";
import visitorsRouter from "./visitors";
import campaignsRouter from "./campaigns";
import trackingRouter from "./tracking";
import adminTrackingProvidersRouter from "./admin-tracking-providers";

const router: IRouter = Router();

router.use(healthRouter);
router.use(authRouter);
router.use(authProfileRouter);
router.use(authGoogleRouter);
router.use(adminRouter);
router.use(adminDashboardRouter);
router.use(adminAnalyticsRouter);
router.use(adminPlatformsRouter);
router.use(adminProductsRouter);
router.use(adminCategoriesRouter);
router.use(adminOrdersHoldsRouter);
router.use(adminOrdersJourneyRouter);
router.use(adminCustomerSessionsRouter);
router.use(adminAnalyticsFunnelRouter);
router.use(meTrackingRouter);
router.use(adminOrdersRouter);
router.use(adminOrdersActionsRouter);
router.use(adminOrdersDiagnoseRouter);
router.use(adminKeysRouter);
router.use(adminClaimsRouter);
router.use(adminDiscountsRouter);
router.use(adminCustomersRouter);
router.use(adminReviewsRouter);
router.use(adminBannersRouter);
router.use(adminPagesRouter);
router.use(adminSettingsRouter);
router.use(adminSettingsExtraRouter);
router.use(adminPaymentProvidersRouter);
router.use(adminWebhooksChatRouter);
router.use(adminAuditRouter);
router.use(adminBalanceRouter);
router.use(adminUpsellRouter);
router.use(adminHomepageRouter);
router.use(adminBrandsRouter);
router.use(adminEmailTemplatesRouter);
router.use(adminTaxRouter);
router.use(adminUsersRouter);
router.use(adminRefundsRouter);
router.use(adminNotificationsRouter);
router.use(adminSeoTrackingRouter);
router.use(adminGiftCardsRouter);
router.use(adminAffiliatesRouter);
router.use(adminAbandonedCartsRouter);
router.use(adminAlertsRouter);
router.use(adminQaRouter);
router.use(adminNewsletterRouter);
router.use(adminFlashSalesRouter);
router.use(adminLoyaltyRouter);
router.use(adminBundlesRouter);
router.use(adminSupportRouter);
router.use(supportTicketsRouter);
router.use(walletRouter);
router.use(adminWalletRouter);
router.use(loyaltyRouter);
router.use(bundlesRouter);
router.use(flashSalesRouter);
router.use(alertsRouter);
router.use(qaRouter);
router.use(newsletterRouter);
router.use(affiliatesRouter);
router.use(abandonedCartsRouter);
router.use(giftCardsRouter);
router.use(couponsRouter);
router.use(ordersRouter);
router.use(orderInvoiceRouter);
router.use(checkoutSessionRouter);
router.use(checkoutOffersRouter);
router.use(orderLookupRouter);
router.use(wishlistRouter);
router.use(currenciesRouter);
router.use(webhooksRouter);
router.use(webhooksStripeRouter);
router.use(webhooksCheckoutRouter);
router.use(checkoutServicesRouter);
router.use(adminCheckoutServicesRouter);
router.use(searchRouter);
router.use(trustpilotRouter);
router.use(socialProofRouter);
router.use(adminSocialProofRouter);
router.use(i18nRouter);
router.use(adminI18nRouter);
router.use(blogRouter);
router.use(adminBlogRouter);
router.use(consentRouter);
router.use(quotesRouter);
router.use(adminQuotesRouter);
router.use(surveysRouter);
router.use(adminJobsRouter);
router.use(adminCircuitBreakerRouter);
router.use(productReviewsRouter);
router.use(pricingRouter);
router.use(adminPriceRulesRouter);
router.use(adminAttributesRouter);
router.use(adminTagsRouter);
router.use(adminUploadRouter);
router.use(adminImportsRouter);
router.use(adminMetenziCatalogRouter);
router.use(adminMetenziMappingsRouter);
router.use(adminMetenziSyncRouter);
router.use(adminMetenziWebhooksRouter);
router.use(adminMappingConflictsRouter);
router.use(adminFeedsRouter);
router.use(productsRouter);
router.use(visitorsRouter);
router.use(campaignsRouter);
router.use(trackingRouter);
router.use(adminTrackingProvidersRouter);

export default router;
